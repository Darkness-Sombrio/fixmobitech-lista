...
const SYSTEM_RESERVED = 17; // GB for System + Emus
...
function renderGames(list) {
    ...
    if(!list || list.length === 0) {
        container.innerHTML = '<div style="text-align:center;width:100%;grid-column:1/-1;padding:40px;color:#666">No hay resultados.</div>';
        GAMES = [];
        return;
    }
    ...
    container.innerHTML = list.map((g, index) => {
        ...
        return `
        <div class="game-card ${isSel ? 'selected' : ''}" data-index="${index}" onclick="toggleGame('${safeName}'); focusedIndex=${index}; updateFocusVisuals()">
...
function handleKeyNavigation(e) {
    ...
    else if (e.key === 'ArrowDown') focusedIndex += 1;
...
function updateStats() {
    const sdInput = document.getElementById('sdSize');
    const sdSize = parseFloat(sdInput.value) || 128;
    ...
    const systemTotal = SYSTEM_RESERVED;
    const totalUsed = gamesTotal + systemTotal;
    const free = sdSize - totalUsed;
    
    // Percentages
    const sysPercent = (systemTotal / sdSize) * 100;
    const gamesPercent = (gamesTotal / sdSize) * 100;
    const totalPercent = Math.min(((totalUsed) / sdSize) * 100, 100);
    
    // Update Text
    document.getElementById('selectedCount').innerText = selected.size;
    document.getElementById('totalSize').innerText = gamesTotal.toFixed(1) + " GB"; // Only games size shown here for clarity
    document.getElementById('freeSpace').innerText = free.toFixed(1) + " GB";
    
    // Update Bar (Advanced Gradient)
    const bar = document.getElementById('sdProgress');
    // Gradient: Grey (Sys) -> Blue (Games) -> Transparent (Free)
    // Stops: 0% -> sys% (Grey), sys% -> total% (Blue)
    const p1 = sysPercent.toFixed(1);
    const p2 = (sysPercent + gamesPercent).toFixed(1);
    
    // Check for overflow
    if(free < 0) {
        bar.style.background = `linear-gradient(90deg, #555 0%, #555 ${p1}%, #ff4444 ${p1}%, #ff4444 100%)`;
        bar.style.width = "100%";
    } else {
        bar.style.background = `linear-gradient(90deg, #555 0%, #555 ${p1}%, var(--neon-blue) ${p1}%, var(--neon-blue) 100%)`;
        bar.style.width = p2 + "%";
    }
    
    document.getElementById('statsText').innerText = 
        `Sistema: ${systemTotal}GB | Juegos: ${gamesTotal.toFixed(1)}GB | Libre: ${free.toFixed(1)}GB`;
}
...
function sendWhatsApp() {
    ...
    const systemTotal = SYSTEM_RESERVED;
    const totalUsed = gamesTotal + systemTotal;
    const free = parseFloat(sd) - totalUsed;
...
*JUEGOS:*
${listText}
📊 *Juegos:* ${selected.size} | 📦 *Peso Juegos:* ${gamesTotal.toFixed(1)} GB
⚙️ *Sistema (Reservado):* ${systemTotal} GB
🧠 *Total Ocupado:* ${totalUsed.toFixed(1)} GB
${free >= 0 ? '✅ Espacio Suficiente' : '⚠️ Falta Espacio (' + free.toFixed(1) + ' GB)'}`;
