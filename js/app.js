...
function renderGames(list) {
    ...
    container.innerHTML = list.map(g => {
        ...
        return `
        <div class="game-card ${isSel ? 'selected' : ''}" onclick="toggleGame('${safeName}')">
...
function toggleGame(name) {
    ...
    // Actualizar visualmente sin re-renderizar todo (más eficiente)
    // Pero como usamos innerHTML masivo, re-renderizamos las clases
    // O mejor, buscamos el elemento especifico
...
function sendWhatsApp() {
    ...
    selList.forEach((n, i) => {
        ...
        total += g[1];
        listText += `${i+1}. ${n} (${g[1]} GB)%0A`;
    });
    
    const free = parseFloat(sd) - total;
    
    const msg = `🎮 *PEDIDO FIXMOBITECH*%0A` +
                `👤 Cliente: *${name}*%0A` +
                `💾 SD: ${sd} GB%0A%0A` +
                `*LISTA DE JUEGOS:*%0A` +
                listText + 
                `%0A📊 Total: ${selected.size} juegos` +
                `%0A📦 Peso: ${total.toFixed(1)} GB` +
                `%0A${free >= 0 ? '✅ CABE EN SD' : '❌ FALTA ESPACIO (' + free.toFixed(1) + ' GB)'}`;
